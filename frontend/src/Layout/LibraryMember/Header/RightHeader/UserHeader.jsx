    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          navigate(`${process.env.PUBLIC_URL}/libraryMemberlogin`);
        } catch (error) {
            Swal.fire({
              icon: 'error',
              title: 'Error!',
  ...
  return (
    <li className="profile-nav onhover-dropdown pe-0 py-0">
      <div className="media profile-media">
        <img src={librarymemberData.profile} loading="lazy" style={{maxWidth: "45px", maxHeight: "40px", borderRadius: "10px"}}/>
      ...
      <UL attrUL={{ className: "simple-list profile-dropdown onhover-show-div" }}>
        <LI
          attrLI={{
            onClick: () => userMenuRedirect(`${process.env.PUBLIC_URL}/library/member/profile`),
          }}>
          <User />
          <span>Account</span>
        </LI>
