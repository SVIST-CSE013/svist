...
const DepartmentLogin = () => {
    ...
    const checkSession = async () => {
      try {
        ...
        if (data.authenticated && data.user.role === 'Department Admin') {
          navigate(`${process.env.PUBLIC_URL}/department/dashboard`);
        } else {
          setLoading(false);
        }
  ...
  const loginAuth = async (e) => {
    ...
    try {
      ...
      if (data.success && data.role === 'Department Admin') {
    ...
    <Fragment>
      <section>
        <Container fluid={true}>
          <Row>
            <Col xl='5' className='b-center bg-size' style={{ backgroundImage: `url(${require('../images/Department_1.jpg')})`, backgroundSize: 'cover', backgroundPosition: 'center', display: 'block' }}>
              <Image attrImage={{ className: 'bg-img-cover bg-center d-none', src: `${require('../images/Department_1.jpg')}`, alt: 'looginpage' }} />
            </Col>
            <Col xl='7 p-0'>
