    ...
    const checkSession = async () => {
      try {
        ...
        if (data.authenticated && data.user.role === 'Faculty') {
          navigate(`${process.env.PUBLIC_URL}/faculty/dashboard`);
        } else {
          setLoading(false);
        }
    ...
    <Fragment>
      <section>
        <Container fluid={true}>
          <Row>
            <Col xl='5' className='b-center bg-size' style={{ backgroundImage: `url(${require('../images/faculty.png')})`, backgroundSize: 'cover', backgroundPosition: 'center', display: 'block' }}>
              <Image attrImage={{ className: 'bg-img-cover bg-center d-none', src: `${require('../images/faculty.png')}`, alt: 'looginpage' }} />
            </Col>
            <Col xl='7 p-0'>
