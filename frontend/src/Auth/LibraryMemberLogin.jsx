...
const LibraryMemberLogin = () => {
    ...
    const checkSession = async () => {
      try {
        ...
        if (data.authenticated && data.user.role === 'LibraryMember') {
          navigate(`${process.env.PUBLIC_URL}/library/member/dashboard`);
        } else {
          setLoading(false);
        }
  ...
  const loginAuth = async (e) => {
    ...
    try {
      ...
      if (data.success && data.role === 'LibraryMember') {
    ...
    <Fragment>
      <section>
        <Container fluid={true}>
          <Row>
            <Col xl='6' className='b-center bg-size' style={{ backgroundImage: `url(${require('../images/library_member.png')})`, backgroundSize: 'cover', backgroundPosition: 'center', display: 'block' }}>
              <Image attrImage={{ className: 'bg-img-cover bg-center d-none', src: `${require('../images/library_member.png')}`, alt: 'loginpage' }} />
            </Col>
            <Col xl='6 p-0'>
                ...
                <div>
                  ...
                  <div className="login-main login-tab">
                    <Form className="theme-form" onSubmit={loginAuth}>
                      <H4>Library Member Login Portal</H4>
...
export default LibraryMemberLogin;
